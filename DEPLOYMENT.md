...
This guide documents deploying a Flask application using the Application Factory pattern and WSGI configuration on PythonAnywhere.
...
os.environ['RATE_LIMIT_DEFAULTS'] = '5 per minute'
os.environ['RATE_LIMIT_STORAGE_URI'] = 'memory://'

# run.py exposes app (created via create_app())
from run import app as application
```

**Important:**
- Replace `yourusername` in `project_home` with your PythonAnywhere username
- **Better approach:** Use PythonAnywhere's **Environment variables** feature in the Web tab instead of hardcoding in the WSGI file
